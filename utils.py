...
def get_boards_loser_indices(moves):
    """
    Return the boards and the indices of the loser's moves.
    """
    boards = [init_board()]
    turns = [1]
    turn = 1
    for move in moves:
        ...
        turns.append(turn)
    if np.sum(boards[-1][0]) < np.sum(boards[-1][1]):
        loser = turn
    else:
        loser = -turn
    loser_indices = [i - 1 for i, t in enumerate(turns) if t == loser]
